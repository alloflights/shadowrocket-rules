/**
 * TikTok 视频无水印下载、信息流广告去除与无视创作者限制保存
 * 原作者: lodepuly, 优化维护: allofights
 */
var watermark = (data) => {
	try {
...
function Follow(data) {
	if (data && data.length > 0) {
		for (let i in data) {
			if (data[i].aweme && data[i].aweme.video) download_list(data[i].aweme);
		}
	}
	return data;
}
...
function Share(aweme_detail) {
	if (aweme_detail && aweme_detail.video) download_list(aweme_detail);
	return aweme_detail;
}
...
function download_list(list) {
	...
	if (list.status) list.status.reviewed = 1;
	if (list.video_control) {
	...
	if (list.aweme_acl && list.aweme_acl.download_general) {
